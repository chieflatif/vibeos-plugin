...
Steps below read framework assets under `.vibeos/` (`decision-engine/`, `reference/`). Profile-driven installs deliberately do not carry them (dormant payload). Resolve every such asset in this order:
...
3. **Plugin root** — the installed VibeOS plugin's own directory (`${CLAUDE_PLUGIN_ROOT}` when that environment variable is set), which contains `decision-engine/` and `reference/` alongside this skill.
